...
def _redact_url(url):
    """Return ``url`` with Trello credentials removed for safe diagnostics.

    Trello accepts credentials both as query parameters (``key`` and ``token``)
    and in token-scoped paths such as ``/tokens/{token}/webhooks``. Exceptions
    include URLs to make debugging failed requests practical, so this helper is
    the last line of defense against accidentally printing reusable API material
    into logs, CI output, or issue trackers. Non-credential query parameters are
    preserved.
    """
    parts = urlsplit(url)
    query = urlencode([
        (key, "REDACTED") if key in ("key", "token") else (key, value)
        for key, value in parse_qsl(parts.query, keep_blank_values=True)
    ])
    path = re.sub(r"(/tokens/)[^/]+", r"\1REDACTED", parts.path)
    return urlunsplit((parts.scheme, parts.netloc, path, query, parts.fragment))


class TrelloClient(object):
    """Base class for Trello REST API access.

    ``TrelloClient`` supports Trello's common key/token authentication as well
    as the package's older OAuth1 flow. For new code, prefer ``api_key`` plus
    ``api_token``. The historic ``api_secret`` parameter remains available as a
    backwards-compatible alias for ``api_token`` only when OAuth1 credentials are
    not supplied.
    """

    def __init__(self, api_key, api_secret=None, token=None, token_secret=None, api_token=None,
                 http_service=requests, proxies=None):
        """Create a Trello API client.

        :param str api_key: API key generated in Trello's Power-Up Admin Portal
            at https://trello.com/power-ups/admin.
        :param str api_token: Token authorized by the Trello user for key/token
            authentication. This is the preferred non-OAuth1 credential.
        :param str api_secret: Deprecated alias for ``api_token`` in key/token
            mode, or the OAuth1 consumer secret when ``token``/``token_secret``
            are supplied. Passing both ``api_secret`` and ``api_token`` in
            key/token mode is rejected to prevent credential mix-ups.
        :param str token: OAuth1 resource owner token returned by
            :func:`trello.util.create_oauth_token`.
        :param str token_secret: OAuth1 resource owner secret paired with
            ``token``.
        :param http_service: Requests-compatible module/session. Tests may pass
            a recorder object with a ``request`` method.
        :param dict proxies: Optional requests proxy mapping. The mapping is
            copied so one client's proxy changes cannot affect another client.
        """

        if api_token is not None and api_secret is not None and token is None and token_secret is None:
            raise ValueError("Pass either api_token or api_secret for key/token auth, not both.")

        query_token = api_token if api_token is not None else api_secret
        ...
        self.proxies = proxies.copy() if proxies is not None else {}
        self.public_only = token is None
        self.api_key = api_key
        self.api_secret = api_secret
        self.api_token = query_token
    ...
    def fetch_json(
            ...
            post_args=None,
            files=None):
        """Fetch JSON from Trello and translate HTTP failures to exceptions.

        ``uri_path`` may be passed with or without a leading slash and is joined
        to ``https://api.trello.com/1``. Caller-provided ``headers`` and
        ``query_params`` are copied before adding defaults, preventing auth
        credentials and user-agent headers from leaking back into caller-owned
        dictionaries. JSON bodies are sent for POST, PUT, and DELETE requests;
        file uploads are passed through unchanged.
        """
        ...
        if response.status_code == 401:
            raise Unauthorized("%s at %s" % (response.text, _redact_url(response.url if hasattr(response, 'url') else url)), response)
        if response.status_code != 200:
            raise ResourceUnavailable("%s at %s" % (response.text, _redact_url(response.url if hasattr(response, 'url') else url)), response)
    ...
    def list_hooks(self, token=None):
        ...
        """
        token = token or self.resource_owner_key or self.api_token
